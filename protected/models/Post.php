...
class Post extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
			...
			array('contacts', 'length', 'max'=>50),
			array('area', 'length', 'max'=>50),
            array('content, title, status', 'safe', 'on'=>'search'),
        );
    }
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'author_id'     => Yii::t('blog','Author'),
	   		'telephone'     => Yii::t('site','Telephone'),
	   		'contacts'     => Yii::t('site','Contacts'),
	   		'area'     => Yii::t('site','Area'),
        );
    }
	...
	public function findPostedInMonth($limit=10)
    {
    	$month = date('n', time());
		$year = date('Y', time());
        return $this->findAll(array(
            'condition'=>'t.status='.self::STATUS_PUBLISHED.' AND t.create_time > '.mktime(0,0,0,$month,1,$year).' AND t.create_time < '.mktime(0,0,0,$month+1,1,$year).'',
            'order'=>'t.views DESC',
            'limit'=>$limit,
        ));
    }
	
	public function findPostedInAll($limit=10)
    {
        return $this->findAll(array(
            'condition'=>'t.status='.self::STATUS_PUBLISHED,
            'order'=>'t.views DESC',
	...
	public function findPostedTuiJian($limit=10)
    {
        return Post::model()->with('comments')->findAll(array(
